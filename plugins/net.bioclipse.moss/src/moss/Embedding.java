...
/*----------------------------------------------------------------------
  ...
  History : 2002.03.11 file created as file submol.java
            ...
            2006.05.03 bug in constructor Embedding(Extension) fixed
...
public class Embedding {
  ...
  /** the array of nodes (references to the underlying graph) */
   ...
   *  <code>CONTAINED</code> in class <code>Graph</code>, which is
   *  used as a special parameter and as a special return value for
   *  the function <code>Graph.embed()</code> in order to save a
  ...
  {                             /* --- create an embedding */
    ...
    if (ext.nodecnt <= 0)       /* if there are no new nodes */
      this.nodes = ext.emb.nodes;
    else {                      /* if there are new nodes */
      this.nodes = new Node[n +ext.nodecnt];
      System.arraycopy(ext.emb.nodes, 0, this.nodes, 0, n);
    }                           /* copy the nodes of the base */
    e = ext.emb.edges.length;   /* get the old number of edges */
    if (ext.edgecnt <= 0)       /* if there are no new edges */
      this.edges = ext.emb.edges;
    else {                      /* if there are new edges */
      this.edges = new Edge[e +ext.edgecnt];
      System.arraycopy(ext.emb.edges, 0, this.edges, 0, e);
    }                           /* copy the edges of the base */
    if (ext.size <= 0) {        /* if single edge or chain extension */
      if (ext.nodecnt > 0) this.nodes[n]   = ext.nodes[1];
      if (ext.edgecnt > 0) this.edges[e]   = ext.edges[0];
      if (ext.edgecnt > 1) this.edges[e+1] = ext.edges[1]; }
    else {                      /* if ring extension */
      for (i = 0; i < ext.size; i++) {
        node = ext.nodes[i];    /* traverse the extension nodes */
        if (node.mark < 0) this.nodes[n++] = node;
        edge = ext.edges[i];    /* traverse the extension edges */
...
public int hashCode ()
  {                             /* --- compute a hash code */
    int  i, k, n, t;            /* loop variables, buffers */
    ...
    for (i = this.nodes.length; --i >= 0; ) {
      ...
      for (k = src.deg; --k >= 0; ) {
        ...
        t   += ((dst.type ^ dst.mark) + edge.type) ^ src.type;
    ...
    for (i = this.edges.length; --i >= 0; ) {
      edge = this.edges[i];     /* traverse the edges */
      t = (edge.src.type ^ edge.src.mark)
        + (edge.dst.type ^ edge.dst.mark);
      h ^= t ^ (t << 11) ^ (t << 19);
