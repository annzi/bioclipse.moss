...
public class Atoms extends TypeMgr {
  ...
  public static final int ELEMMASK    = 0x007f;
  /** the flag indicating whether an atom is part of an aromatic ring */
  public static final int AROMATIC    = 0x0080;
  /** the mask for extracting the charge from an atom type */
  public static final int CHARGEMASK  = 0x1f00;
  ...
  static {                      /* --- class initialization */
    ...
    for (i = ELEMCNT; --i > 0; ) {
      ...
      k = (int)name.charAt(0);  /* skip special names (no letter) */
      if ((k < 'A') || (k > 'Z')) continue;
      m = (name.length() > 1) ? (int)(name.charAt(1) -'a') : 26;
      Atoms.map[k-'A'][m] = i;  /* set up the element name map */
  ...
  {                             /* --- get an atom code */
    ...
    ||  (c == '-')) {           /* check for a digit */
      ...
      code |= Atoms.codeCharge((c == '+') ? e -'0' : '0' -e);
      c = (k < len) ? name.charAt(k) : 0;
